...
\begin{document}
	...
	\begin{tikzpicture}[>=stealth, scale=1.2]
		...
		% Fricción viscosa del péndulo
		\draw[->, very thick, red!80!black] (\x, 2*\Rrueda + \H) ++(45:0.8) arc (45:135:0.8) node[left, xshift=-2pt] {\large $c_\theta \dot{\theta}$};
		
		% --- 5. INDICADORES DE MOVIMIENTO (MODIFICADO A ANTIHORARIO) ---
		\draw[dashed, thick, gray] (Pivote) -- ++(0, \L + 0.5);
		
		% La flecha comienza en la barra y termina en la vertical (90 grados)
		\draw[->, thick, gray] ({\x + 1.5*sin(\ang)}, {2*\Rrueda + \H + 1.5*cos(\ang)}) 
