...
\begin{document}
	...
	\begin{tikzpicture}[>=stealth, scale=1.2]
		...
		\node at (\x+0.5, 2*\Rrueda + \H/2) {\Large $M$}; % Desplazado ligeramente para que no pise la línea
		...
		% Acción de control u (Negro, grueso)
		...
		% Fricción del carro (asumiendo vel. x positiva)
		\draw[->, very thick, red!80!black] (\x - \W/2, 2*\Rrueda + \H/4) -- ++(-1.5, 0) node[above] {\large $c_x \dot{x}$};
		
		% Fricción viscosa del péndulo (Par en el pivote)
		% Como theta positivo es horario, el par de fricción se dibuja antihorario
		\draw[<-, very thick, red!80!black] (\x, 2*\Rrueda + \H) ++(45:0.8) arc (45:150:0.8) node[above left, xshift=5pt] {\large $c_\theta \dot{\theta}$};
		
		% --- 5. INDICADORES DE MOVIMIENTO (Opcional, para dar contexto) ---
		% Ángulo theta original en gris clarito para no saturar
