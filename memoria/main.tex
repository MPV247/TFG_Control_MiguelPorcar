...
Para mejorar la comprensión de los requisitos que requiere el proyecto se han dividido en requisitos funcionales y requisitos no funcionales. 
...
\begin{itemize}
	\item \textbf{Modelado y simulación:} El entorno de simulación (Matlab/Simulink) debe ser un entorno de prueba y desarrollo de algoritmos de control como paso previo a su implementación en hardware real. Se debe permitir el modelado de la dinámica no lineal y la introducción de los parámetros (masas, inercias, rozamientos...) que caracterizan el sistema real. 

	\item \textbf{Control de inyección de energía (Swing-up):} El sistema de control debe ser capaz de aplicar técnicas no lineales para conmutar la posición del péndulo hasta la región de estabilización (completamente invertido).
	
	\item \textbf{Control de estabilización (Realimentación del estado):} Una vez el péndulo se encuentre en la región lineal cercana a la vertical, el sistema debe conmutar a un controlador por realimentación del estado para mantener la posición del péndulo y ubicar el carro en el centro del riel.
	
	\item \textbf{Adquisición de señales y actuación:} El microcontrolador del prototipo físico debe ser capaz de leer las señales de entrada (encoders), calcular la velocidad y la posición de ambas articulaciones así como aplicar acciones de control mediante señales PWM (Pulse Width Modulation).  
